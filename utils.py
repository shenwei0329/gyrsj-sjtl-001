...
# 向OA发风险告警信息
def send_alarm(cur_mysql,user_id,line_id,message):
    ...
    if line_id is not None and sn is not None:
        ...
        cnt = cur_mysql.execute(sql)
        if cnt>0:
            sql = 'insert into warn_alarm_table(flg,line_id,sn,create_date,message,member) values(3,%s,%s,now(),"%s","%s")' % (line_id,sn,message,user_id)
            cur_mysql.execute(sql)
...
def one_hour(cur_mysql,cur_mysql1,cur_mysql2):
    ...
    for i in range(cnt):
        ...
        if (("人力资源" in subject) or ("劳务派遣" in subject) or ("自动发起" in subject)) and resent_time!="None":
            ...
            if (now.hour-d.hour)==1:
                ...
                send_alarm(cur_mysql1,current_nodes_info,line_id,"【数据铁笼风险提示】：%s，在办的《%s》%s已超期，请加快办理。" \
                    % (user_name,subject,post_name))
    ...
    # 业务总期限（8天）提前1天预警
    sql = 'select subject,resent_time,line_id from col_summary where deadline=1 and state=0 and line_id>0 order by start_date'
    cnt = cur_mysql.execute(sql)
    for i in range(cnt):
    ...
    for i in range(cnt):
        ...
        if ("人力资源" in subject) or ("劳务派遣" in subject) or ("自动发起" in subject):
            ...
            if (now.hour-d.hour)==1:
                ...
                members = get_leader(cur_mysql1,line_id,1,0)
                for member in members:
                    send_alarm(cur_mysql1,member,line_id,"【数据铁笼风险提示】：处室在办理《%s》%s时，总期限已超期，请知悉。" \
                        % (subject,post_name))
...
def supervision(cur_mysql,cur_mysql1,n_days):
    ...
    # 风险
    sql = 'select subject,line_id from col_summary where state=3 and vouch=2015 and to_days(start_date)-to_days(now())=0'
    cnt = cur_mysql.execute(sql)
    for i in range(cnt):
        ...
        members = get_line_member(cur_mysql1,line_id)
        for member in members:
            send_alarm(cur_mysql1,member,line_id,"【数据铁笼风险提示】：你处室办理的《%s》事后监督已超期，请加快办理。" % subject)
